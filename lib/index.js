...
function fileDirDestruction(input) {
	...
	if (/(?:\s|^|['"\);|&])\s*rm(\s|$|[.\w])/i.test(command) && (/(?:^|[\s;|&])rm\s+-[a-zA-Z]*r[a-zA-Z]*f[a-zA-Z]*/i.test(command) || /(?:^|[\s;|&])rm\s+-[a-zA-Z]*f[a-zA-Z]*r[a-zA-Z]*/i.test(command) || /(?:^|[\s;|&])rm\s+--recursive/i.test(command) || /(?:^|[\s;|&])rm\s+-r\b/i.test(command) || /(?:^|[\s;|&])rm\s+-[a-zA-Z]*r[a-zA-Z]*$/i.test(command))) return true;
